...
use disturb_shared::{BallId, ClientMessage, Position, ServerMessage, TargetVelocity, Velocity};
...
fn server_setup_system(mut net: ResMut<NetworkResource>) {
    let socket_address: SocketAddr = "0.0.0.0:5222".parse().expect("cannot parse ip");
    let webrtc: SocketAddr = "0.0.0.0:5224".parse().expect("cannot parse ip");
    println!("Listening... {:?}", socket_address);
    net.listen(socket_address, Some(webrtc.clone()), Some(webrtc));
}
